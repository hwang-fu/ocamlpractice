...
\begin{itemize}
  \item \textbf{Choose the most constrained cell} (fewest candidates
    remaining), known as the \emph{minimum remaining values} rule,
    \textbf{MRV} for short. This is the same rescue-the-scarce principle
    as Warnsdorff's onward counts: a cell with two candidates gives a
    wrong guess a 50\% chance of instant refutation.
...
the solver wraps it in MRV-guided backtracking. Beyond the solved grid,
the solver also returns its whole reasoning as a list of typed events,
one per action, which the renderer replays:

\begin{center}
\begin{tabular}{ll}
  \texttt{Given} & a starting digit is placed on the board \\
  \texttt{Naked} & a naked single fires (section 2) \\
  \texttt{Hidden} & a hidden single fires in the named unit (section 2) \\
  \texttt{Guess} & propagation stalled; a candidate is tried at the MRV
                   cell (section 3) \\
  \texttt{Contradiction} & a cell lost its last candidate; this branch is
                           dead (section 3) \\
  \texttt{Backtrack} & the search rewinds to the latest guess and tries
                       differently (section 3) \\
\end{tabular}
\end{center}

The renderer draws what solvers on paper actually do: pencil marks
shrinking as eliminations cascade, deduced digits in one color, guesses
in another, and a contradiction flashing before its branch rewinds.
Constraint propagation made watchable.

\paragraph{Generating puzzles.}
The generator runs the machinery backwards. It first builds a random
solved grid (the solver on an empty board, choices shuffled), then digs:
visiting the cells in random order, it blanks each one and asks whether
the puzzle still has exactly one solution, restoring the digit when it
does not. The uniqueness question is answered by a solver variant that
keeps searching after the first solution and stops at the second. What
remains is proper (uniquely solvable) and minimal (every surviving given
is load-bearing), typically 22 to 27 givens. Difficulty is graded by how
the solver cracks the result: naked singles alone is easy, hidden singles
needed is medium, guessing required is hard.
