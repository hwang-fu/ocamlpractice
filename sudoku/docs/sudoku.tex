...
is not yet the answer; the rules at hand are exhausted. Here is a stalled
row, and what a guess does to it:
...
\smallskip
No single rule fires above --- every unresolved cell has two candidates,
every missing digit two homes. Guessing $2$ strips $2$ from the middle
cell's candidates, leaving a naked single $4$; placing it leaves $7$ ---
the cascade runs again.
\end{center} The solver then guesses: pick an unresolved cell, try one of its
candidates, propagate, and recurse; a contradiction (some cell left with
no candidates) refutes the guess, and backtracking tries the next one.
This is the familiar search tree --- with two twists that keep it tiny
compared to the tours and queens of earlier quizzes:

\begin{itemize}
  \item \textbf{Choose the most constrained cell} (fewest candidates
    remaining), the \emph{minimum remaining values} rule --- the same
